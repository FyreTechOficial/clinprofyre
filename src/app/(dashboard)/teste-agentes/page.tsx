    ...
    <div className="animate-fade-in flex flex-col h-[calc(100vh-4rem)] -mx-4 -my-6 lg:-mx-8 lg:mx-0 lg:my-0 lg:h-[calc(100vh-7rem)]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-hairline bg-canvas">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-ink">Teste de Agentes</h1>
            <p className="text-xs text-ink-secondary">Simula mensagens sem usar WhatsApp real</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="hidden sm:flex items-center gap-3 mr-2">
            <div className="flex items-center gap-1 text-xs">
              <MessageSquare className="h-3.5 w-3.5 text-ink-tertiary" />
              <span className="text-ink-secondary">{stats.total}</span>
            </div>
            <div className="flex items-center gap-1 text-xs">
          ...
          <button
            ...
            className={cn(
              ...
              showConfig ? "bg-brand-50 border-brand-200 text-brand-700" : "bg-canvas border-divider text-ink-secondary hover:bg-parchment"
            )}
          >
          ...
          <button
            onClick={clearChat}
            className="inline-flex items-center gap-1.5 rounded-xl border border-divider bg-canvas px-3 py-2 text-xs font-medium text-ink-secondary hover:bg-parchment transition-all"
          >
      ...
      {/* Config panel */}
      {showConfig && (
        <div className="px-4 py-3 border-b border-hairline bg-parchment/80 animate-slide-up">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-3xl">
            <div>
              <label className="block text-[10px] font-bold text-ink-secondary uppercase tracking-wider mb-1">Instância Evolution</label>
              <input
                type="text"
                value={config.instanceName}
                onChange={(e) => setConfig({ ...config, instanceName: e.target.value })}
                className="w-full rounded-lg border border-divider px-3 py-1.5 text-sm font-mono focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-100"
              />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-ink-secondary uppercase tracking-wider mb-1">Telefone Simulado</label>
              <input
                type="text"
                value={config.phone}
                onChange={(e) => setConfig({ ...config, phone: e.target.value })}
                className="w-full rounded-lg border border-divider px-3 py-1.5 text-sm focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-100"
              />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-ink-secondary uppercase tracking-wider mb-1">Nome do Paciente</label>
              <input
                type="text"
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            <span className="text-[10px] text-ink-secondary">
              Webhook: <span className="font-mono text-ink">{process.env.NEXT_PUBLIC_N8N_URL || "n8n.fyreoficial.com.br"}/webhook/clinpro-webhook</span>
            </span>
          </div>
        </div>
      )}
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 py-4 bg-parchment/30" style={{ backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23e5e7eb' fill-opacity='0.2'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E\")" }}>
        <div className="max-w-3xl mx-auto space-y-3">
          {messages.length === 0 && (
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-bold text-ink">Ambiente de Teste</h3>
              <p className="text-sm text-ink-tertiary mt-1 max-w-md mx-auto">
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-2">
                {["Olá, tudo bem?", "Quero agendar uma consulta", "Quanto custa implante?", "Vocês atendem sábado?"].map((suggestion) => (
                  <button
                    ...
                    className="rounded-full border border-divider bg-canvas px-3 py-1.5 text-xs text-ink-secondary hover:bg-brand-50 hover:border-brand-200 hover:text-brand-700 transition-all"
                  >
                    {suggestion}
                  </button>
                ))}
              ...
              <div className={cn("flex gap-2", msg.role === "user" ? "justify-end" : "justify-start")}>
                {msg.role !== "user" && (
                  <div className={cn(
                    ...
                    msg.status === "sending" ? "bg-amber-100" : "bg-parchment"
                  )}>
                ...
                <div className={cn(
                  ...
                  msg.role === "assistant" ? "bg-canvas border border-hairline text-ink rounded-tl-md shadow-sm" :
                  ...
                  <p className={cn("text-[10px] mt-1", msg.role === "user" ? "text-brand-200" : "text-ink-tertiary")}>
                ...
                {msg.role === "user" && (
                  <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-parchment flex items-center justify-center">
              ...
              {msg.debug && (
                <div className={cn("mt-1", msg.role === "user" ? "mr-10 text-right" : "ml-10")}>
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-[10px] text-ink-tertiary hover:text-brand-600 transition-colors"
                  >
                    {expandedDebug === msg.id ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                    Debug
                  </button>
                  {expandedDebug === msg.id && (
                    <div className="mt-1 rounded-lg bg-gray-900 text-ink-tertiary p-3 text-[11px] font-mono overflow-x-auto text-left relative">
                      <button
                        onClick={() => copyDebug(msg.debug)}
                        className="absolute top-2 right-2 rounded-md p-1 text-ink-secondary hover:text-white hover:bg-gray-700 transition-colors"
                      >
      ...
      <div className="border-t border-hairline bg-canvas px-4 py-3">
        <div className="max-w-3xl mx-auto flex items-end gap-2">
          <div className="flex-1 flex items-end gap-2 rounded-2xl border border-divider bg-parchment/50 p-2 transition-all focus-within:border-brand-300 focus-within:ring-2 focus-within:ring-brand-100 focus-within:bg-canvas">
            <textarea
              ref={textareaRef}
              value={input}
              ...
              rows={1}
              disabled={sending}
              className="flex-1 resize-none bg-transparent px-2 py-1.5 text-sm text-ink placeholder:text-ink-tertiary outline-none disabled:opacity-50"
            />
            <button
              onClick={handleSend}
